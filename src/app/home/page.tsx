...
const Home = () => {
  ...
  const [searchTerm, setSearchTerm] = useState("https://shopee.vn/search?keyword=robot");
  const handleURL = async () => {
    ...
    try {
      ...
      const items = $('a.contents').toArray().map((item) => item.attribs.href);
      console.log(items);
    } catch (error) {
    ...
    <>
      ...
      <div className="flex flex-col items-center p-4">
        <div className="w-full max-w-md flex">
          <Input
            type="text"
            placeholder="Input a Shopee URL..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <Button className="ml-2" onClick={handleURL}>
            Add
          </Button>
          <Button className="ml-2" onClick={handleClear}>
            Clear
          </Button>
          <Button className="ml-2" onClick={handleExport}>
            Export Excel
          </Button>
        </div>
